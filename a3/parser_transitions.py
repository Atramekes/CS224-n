...
def minibatch_parse(sentences, model, batch_size):
    ...
    while(unfinished_parses):
        predict_transitions = model.predict(unfinished_parses[:batch_size])
        del_list = []
        for now_parse, predict_transition in zip(unfinished_parses[:batch_size], predict_transitions):
            now_parse.parse_step(predict_transition)
            if (not now_parse.buffer and len(now_parse.stack) == 1):
                unfinished_parses.remove(now_parse)
